...
function DashboardPage({ user, setUser, onNav }) {
    ...
    if (!user) return (
        <div className="text-center py-24"><div className="text-6xl mb-5">🔒</div>
            ...
            <button onClick={() => onNav('login')} className="bg-yellow-500 text-black font-black px-7 py-3 rounded-2xl mt-2">Sign In</button></div>
    );
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-8">
            <div className="flex items-center gap-5 mb-8 bg-gray-800 rounded-2xl p-6">
                <div className="w-16 h-16 bg-yellow-500 rounded-full flex items-center justify-center text-black font-black text-2xl">{user.name[0]}</div>
                <div>
            ...
            <div className="flex gap-2 mb-6 flex-wrap">
                {tabs.map(([t, l]) => (
                    <button key={t} onClick={() => setTab(t)}
                        className={`px-4 py-2 rounded-xl text-sm font-semibold ${tab === t ? 'bg-yellow-500 text-black' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`} style={{ transition: 'all .15s' }}>
                        {l}
                    </button>
                ))}
            </div>
            ...
            {tab === 'profile' && (
                <div className="bg-gray-800 rounded-2xl p-6">
                    <div className="flex justify-between items-center mb-5">
                        ...
                        <button onClick={() => setEditMode(!editMode)} className="text-yellow-400 text-sm hover:text-yellow-300">{editMode ? 'Cancel' : '✏️ Edit'}</button>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        {[['Name', editMode ? <input value={name} onChange={e => setName(e.target.value)}
                            className="bg-gray-700 text-white px-3 py-1.5 rounded-xl border border-gray-600 focus:outline-none focus:border-yellow-500 text-sm w-full" /> : user.name],
                    ...
                    {editMode && <button onClick={() => { setUser({ ...user, name }); setEditMode(false); showToast?.('Profile updated!'); }} className="mt-5 bg-yellow-500 hover:bg-yellow-400 text-black font-black px-6 py-2.5 rounded-xl text-sm">Save Changes</button>}
                </div>
            )}
                        ...
                        <div key={o.id} className="bg-gray-800 rounded-2xl p-5">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <div className="text-right">
                                    <p className="text-yellow-400 font-black text-lg mb-1">${o.total}</p>
            ...
            {tab === 'wishlist' && (
                <div className="text-center py-14 text-gray-500">
                    ...
                    <button onClick={() => onNav('shop')} className="bg-yellow-500 text-black font-black px-6 py-3 rounded-2xl">Browse Products</button>
                </div>
            )}
                ...
                <div className="bg-gray-800 rounded-2xl p-6 space-y-3">
                    ...
                    {[['🔑 Change Password', 'Update your account password', '→'], ['📱 Two-Factor Auth', 'Add an extra layer of security', 'Enable →'], ['🌐 Active Sessions', 'Manage devices signed into your account', 'View →'], ['🗑 Delete Account', 'Permanently delete your account', 'Delete →']].map(([t, d, a]) => (
                        <div key={t} className="flex items-center justify-between bg-gray-700/50 hover:bg-gray-700 rounded-2xl p-4" style={{ transition: 'background .15s' }}>
                            ...
                            <button className={`text-sm font-semibold ${a.includes('Delete') ? 'text-red-400 hover:text-red-300' : 'text-yellow-400 hover:text-yellow-300'}`} style={{ transition: 'color .15s' }}>{a}</button>
                        </div>
                    ))}
                </div>
            )}
