...
function ProductDetailPage({ product, onAdd, onNav }) {
    ...
    if (!product) return (
        <div className="text-center py-20 text-gray-400">
            ...
            <button onClick={() => onNav('shop')} className="mt-3 text-yellow-400 hover:underline text-sm">Back to Shop</button>
        </div>
    );
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-8">
            <button onClick={() => onNav('shop')} className="text-gray-400 hover:text-yellow-400 text-sm mb-6 flex items-center gap-1 font-semibold" style={{ transition: 'color .15s' }}>← Back to Shop</button>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2 mb-4">
                        <Stars r={product.rating} /><span className="text-yellow-400 font-bold">{product.rating}</span>
                        <span className="text-gray-500 text-sm">({product.reviews} reviews)</span>
                    </div>
                    <div className="flex items-baseline gap-3 mb-5">
                        <span className="text-5xl font-black text-yellow-400">${product.price}</span>
                        {product.disc > 0 && <>
                    ...
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <button onClick={() => { for (let i = 0; i < qty; i++)onAdd(product); }} disabled={product.stock === 0}
                            className="flex-1 bg-yellow-500 hover:bg-yellow-400 disabled:opacity-40 text-black font-black py-3 rounded-xl text-lg" style={{ transition: 'background .15s' }}>
                ...
                <div className="flex gap-6">
                    {[['desc', 'Description'], ['reviews', 'Reviews'], ['shipping', 'Shipping']].map(([k, l]) => (
                        <button key={k} onClick={() => setActiveTab(k)}
                            className={`pb-3 text-sm font-semibold border-b-2 -mb-px ${activeTab === k ? 'border-yellow-500 text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`} style={{ transition: 'all .15s' }}>
                            {l}
                        </button>
                    ))}
                </div>
                        ...
                        <div key={r.user} className="bg-gray-800 rounded-xl p-5">
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-3">
                                    <div className="w-9 h-9 bg-yellow-500/20 rounded-full flex items-center justify-center text-yellow-400 font-bold">{r.user[0]}</div>
